...
defmodule FsetWeb.IconView do
  ...
  def ticket(opts \\ []) do
    ...
    svg_tag("feature", path, opts)
  end

  defp svg_tag(title, path, opts) do
    assigns = %{
      viewbox: opts[:viewbox] || "0 0 24 24",
      class: opts[:class] || "w-6 h-6",
      title: opts[:title] || title,
      path: path
    }

    ~H"""
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox={@viewbox} class={@class} stroke="currentColor">
      <title><%= @title %></title>
      <%= @path %>
    </svg>
    """
