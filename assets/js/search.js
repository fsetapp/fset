...
const typeSearch = (selector, modelsGetter, opts = {}) => {
  ...
  return new autoComplete({
    ...comboboxOpts,
    selector: selector,
    data: { src: modelsGetter(modelsToDataList), keys: ["display"] },
  });
};
    ...
    postSelection: (target, feedback) => {
      target.dispatchEvent(
        new CustomEvent("search-selected", {
          detail: { selector, value: feedback.selection.value },
        }),
      );
    },
  });
    ...
    class extends HTMLElement {
      ...
      createAutoComplete() {
        ...
        switch (this.getAttribute("list")) {
          case "typesearch":
            if (input.id)
              this._autocomplete ||= typeSearch(`#${input.id}`, dataList, {
                placeHolder: this.getAttribute("placeholder"),
                only: input.opts.allowedTs,
              });
            break;
          case "jumptotype":
            if (input.id)
              this._autocomplete ||= projectSearch(`#${input.id}`, dataList, {
                placeHolder: this.getAttribute("placeholder"),
              });
            break;
        }
    ...
    events: {
      input: {
        ...
        navigate: function (e) {
          ...
          let option = list.querySelector(
            `[id='${this.getAttribute("aria-activedescendant")}']`,
          );
...
const dataList = (data, opts = {}) => {
  ...
  for (let i = 0; i < anchors.length; i++) {
    ...
    if (!opts.only)
      datalist.push(
        Object.assign({ anchor: anchors[i], fmodelname: fmodelname }, fmodel),
      );
    else if (containsRefTo(opts.only, fmodel))
      datalist.push(
        Object.assign({ anchor: anchors[i], fmodelname: fmodelname }, fmodel),
      );
  }
