  ...
  customElements.define(
    ...
    class extends HTMLElement {
      ...
      connectedCallback() {
        ...
        this.on(
          "tree-command",
          buffer(this.handlePostTreeCommand.bind(this), 5),
        );
      ...
      handleRemoteConnected(e) {
        ...
        this._store = Store.fromProject(project, { imports: [Model] });
        ...
        channel.on("each_batch", ({ batch }) => {
          for (let file of batch) this._store.fields.push(file);
        });
          ...
          File.FileTree(
            {
              target: "[id='project']",
              fileBody: "file-body",
              select: decodeURIComponent(`[${currentFile?.key}]`),
            },
            this._store,
          );
          File.changeFile({
            projectStore: this._store,
            tree: { _passthrough: { fileBody: "file-body" } },
            filename: currentFile?.key,
            fmodelname: decodeURIComponent(location.hash.replace("#", "")),
          });
        ...
        Remote.taggedDiff(this._store, (diff) => {
          channel
            ...
            .receive("error", (reasons) =>
              console.log("update project failed", reasons),
            )
            .receive("noop", (a) => a)
      ...
      diffRender(opts = {}) {
        Object.defineProperty(this._store, "_diffToRemote", {
          value: this.runDiff(),
          writable: true,
        });
      ...
      handleSchUpdate(e) {
        ...
        if (sch)
          channel
            .push("push_sch_meta", { $a: sch.$a, metadata: sch.metadata })
      ...
      handleSearchSelected(e) {
        ...
        File.changeFile({
          ...
          tree: { _passthrough: { fileBody: "file-body" } },
          filename,
          fmodelname: decodeURIComponent(`[${fmodelname}]`),
          focus: true,
        });
        File.FileTree(
          {
            target: "[id='project']",
            fileBody: "file-body",
            select: decodeURIComponent(`[${filename}]`),
            focus: false,
          },
          this._store,
        );
      ...
      pushChanged() {
        for (let commbobox of this.querySelectorAll("combo-box"))
          commbobox.dispatchEvent(
            new CustomEvent("data-push", {
              detail: { _models: Store.Indice.anchorsModels(this._store) },
            }),
          );
      }
      changeUrl() {
        let file = document.querySelector("[id='project'] [role='tree']")
          ?._walker?.currentNode;
        let fileBodyNode = document.querySelector("file-body [role='tree']")
          ?._walker?.currentNode;
        ...
        switch (true) {
          case !!(fileIsFile && file.key) && !!(notFileNode && fileBodyNode.id):
            history.replaceState(
              null,
              "",
              `${window.project_path}/m/${encodeURIComponent(file.key)}#${fileBodyNode.id}`,
            );
            break;
          case !!(fileIsFile && file.key):
            history.replaceState(
              null,
              "",
              `${window.project_path}/m/${encodeURIComponent(file.key)}`,
            );
            break;
        }
      }
      ...
      handleAction(e) {
        ...
        switch (e.target.closest("[data-tree-action]")?.dataset?.treeAction) {
          case "mark_as_main":
            ...
            break;
          case "move_up":
            this.keydown({ key: "ArrowUp", altKey: true });
            break;
          case "move_down":
            this.keydown({ key: "ArrowDown", altKey: true });
            break;
          case "clone":
            this.keydown({ key: "ArrowDown", shiftKey: true, altKey: true });
            break;
          case "copy":
            this.keydown({ key: "c", metaKey: true });
            break;
          case "cut":
            this.keydown({ key: "x", metaKey: true });
            break;
          case "paste":
            this.keydown({ key: "v", metaKey: true });
            break;
          case "delete":
            this.keydown({ key: "Delete" });
            break;
          case "add_item":
            this.keydown({ key: "+", shiftKey: true });
            break;
        }
      }
